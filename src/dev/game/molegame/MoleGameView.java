...
/*			usernameEditText.setText(rankingSettings.getString(
	...
	public void saveLevel(int score){
		...
		if(mLevel ==level && 
				mLevel < Constants.LEVELS && 
				score >= Constants.QULIFICATION[level]){
	...
	public void restartGame() {
		mUIThread = new GameThread(this.getHolder(),
				this.getContext(),
				mHandler);
	...
	private void initRes() {
		mBgImage = BitmapFactory.decodeResource(mContext.getResources(),
				R.drawable.game_bg1);
		mHoleImage = mContext.getResources().getDrawable(
				R.drawable.root);
		...
		mSparkImage = mContext.getResources().getDrawable(
				R.drawable.light);

		Resources res = mContext.getResources();
		
		moleMap = new HashMap<Integer,Drawable>();
		moleMap.put(0, mContext.getResources().getDrawable(
				R.drawable.blue_mole));
		moleMap.put(1, mContext.getResources().getDrawable(
				R.drawable.dark_blue_mole));
		moleMap.put(2, mContext.getResources().getDrawable(
				R.drawable.green_mole));
		moleMap.put(3, mContext.getResources().getDrawable(
				R.drawable.killer_mole));
		moleMap.put(4, mContext.getResources().getDrawable(
				R.drawable.purple_mole));
		moleMap.put(5, mContext.getResources().getDrawable(
				R.drawable.red_mole));
		moleMap.put(6, mContext.getResources().getDrawable(
				R.drawable.bomb));
		...
		soundPoolMap.put(UIModel.EFFECT_FLAG_HIT, soundPool.load(getContext(),
				R.raw.hit, 1));
	}
		...
		public void run() {
			...
			//		handleEffect(mUIModel.getEffectFlag());
				...
				if (mUIModel.getStatus() == UIModel.GAME_STATUS_GAMEOVER) {
					...
					bundle.putInt(MoleGameView.HANDLE_MESSAGE_FINAL_RECORD,
...
/*			
			...
			canvas.drawText(uiModel.getHitCount(), 5, (float) 15
...
/*			Log.d("x:-------------",Float.toString(mPaintArea.mMaxX - 5));*/
		...
		public Bitmap initUIModel(RectArea paintArea) {
			...
			Bitmap bmp = Bitmap.createBitmap(
					paintArea.mMaxX, paintArea.mMaxY, Config.ARGB_8888);
		...
		public void checkSelection(int x, int y) {
			if(mUIModel.checkSelection(x, y)){
				drawSpark = true;
...
/*			if (mSoundsFlag) {
		...
		private void playSoundEffect(int soundId) {
			try {
				...
				soundPool.play(soundPoolMap.get(soundId), 100, volume, 1, 0,
						1f);
			} catch (Exception e) {
